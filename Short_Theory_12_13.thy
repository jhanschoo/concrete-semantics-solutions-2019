...
inductive
  ...
  where
    ...
    OrLeft: "(c\<^sub>1,s) \<Rightarrow> s' \<Longrightarrow> (c\<^sub>1 OR c\<^sub>2, s) \<Rightarrow> s'" |
    OrRight: "(c\<^sub>2,s) \<Rightarrow> s' \<Longrightarrow> (c\<^sub>1 OR c\<^sub>2, s) \<Rightarrow> s'"
...
lemmas [simp] = hoare.Skip hoare.Assign hoare.Seq hoare.If hoare.Or

lemmas [intro!] = hoare.Skip hoare.Assign hoare.Seq hoare.If hoare.Or
...
lemma wp_Or[simp]:
  "wp (c\<^sub>1 OR c\<^sub>2) Q =
  (\<lambda>s. wp c\<^sub>1 Q s \<and> wp c\<^sub>2 Q s)"
  by (rule ext) (auto simp: wp_def)
...
next
  ...
  qed
next
  case Or thus ?case by (auto intro: conseq)
qed auto
