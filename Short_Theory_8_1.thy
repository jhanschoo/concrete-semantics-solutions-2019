...
lemma exec1I [intro, code_pred_intro]:
  "\<lbrakk>c' = iexec (P !! i) (i, s, stk);
    0 \<le> i;
    i < size P
  \<rbrakk> \<Longrightarrow> P \<turnstile> (i, s, stk) \<rightarrow> c'"
...
lemma exec_appendL_if [intro]:
  "\<lbrakk>size P' \<le> i;
    P \<turnstile> (i - size P', s, stk) \<rightarrow>* (j, s', stk');
    i' = size P' + j
  \<rbrakk> \<Longrightarrow> P' @ P \<turnstile> (i, s, stk) \<rightarrow>* (i', s', stk')"
  by (drule exec_appendL [where P'=P']) simp

lemma exec_append_trans[intro]:
  "\<lbrakk>P \<turnstile> (0, s, stk) \<rightarrow>* (i', s', stk');
    size P \<le> i';
    P' \<turnstile>  (i' - size P, s', stk') \<rightarrow>* (i'', s'', stk'');
    j'' = size P + i''
   \<rbrakk> \<Longrightarrow> P @ P' \<turnstile> (0, s, stk) \<rightarrow>* (j'', s'', stk'')"
...
lemma bcomp_correct[intro]: "0 \<le> n \<Longrightarrow>
...
next
  ...
  moreover have "?cw \<turnstile> (size ?cb + size ?cc, s2, stk) \<rightarrow>* (0, s2, stk)"
    by fastforce
  moreover have "?cw \<turnstile> (0, s2, stk) \<rightarrow>* (size ?cw, s3, stk)" by (rule WhileTrue(5))
...
lemma isuccs_LOADI_correct: "{fst c' | c' i s stk. c' = iexec (LOADI i) (n, s, stk)} = isuccs (LOADI i) n"
  by (auto simp add: isuccs_def)
